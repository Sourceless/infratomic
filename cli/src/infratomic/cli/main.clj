...
(defn- post-json
  "POST `body` to `url` with a JSON content type, returning the response
  body as a string."
  [url body]
  (let [client  (HttpClient/newHttpClient)
        request (-> (HttpRequest/newBuilder (URI/create url))
                     ...
                     .build)]
    (.body (.send client request (HttpResponse$BodyHandlers/ofString)))))
...
(defn- apply-gated!
  ...
  JSON to `policy-check-url` -> branch on the response's `violations`:
  print each and exit non-zero without ever invoking real `terraform
  apply` if any are present, else invoke real `terraform apply tfplan` and
  pass through its exit code. Returns the exit code the CLI process should
  exit with."
  [policy-check-url]
  (let [plan-exit (terraform! ["plan" "-out=tfplan"])]
    (if-not (zero? plan-exit)
      plan-exit
      (let [{:keys [exit out]} (capture! ["show" "-json" "tfplan"])]
        (if-not (zero? exit)
          exit
          (let [{:strs [violations]} (json/parse-string (post-json policy-check-url out))]
            (if (seq violations)
              (do
