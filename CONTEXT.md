...
**Drift**:
An out-of-band change to a Terraform-managed resource: a live value in LocalStack diverging from what Terraform itself last asserted for it, in one of three forms — plain attribute-level drift (below), New-Child Drift, or Removed-Child Drift. Attribute-level drift is detected by the drift Rule (`query.clj`), a query-time-only function — deliberately never registered in `policy.clj`'s Rule registry — that finds every managed resource whose most recent Write Source is `:sync` and compares its current attributes against its stored values as of the most recent `:terraform`-sourced write (found via Datomic history, `d/history`/`d/as-of` — see ADR-0009). All three forms are merged into one flat list, surfaced via `GET /drift` and the CLI's `drift-check` subcommand. Distinct from a Discovered Resource: a Discovered Resource was never Terraform-managed in the first place, whereas Drift is a Terraform-managed resource (or, for New-/Removed-Child Drift, one of its FK-bearing children) whose live value has since diverged. Detection only — Drift itself is never mutated or reverted by this mechanism, and it is never evaluated as part of the pre-apply Policy Check. As of issue #34, a drifted resource that also violates a registered policy Rule is no longer left untouched forever: Reconciliation's `apply!` remediation path is triggered by exactly this condition (managed + drifted + policy-violating) — Drift detection itself still does nothing but detect; the *acting* on a drifted, violating resource is Reconciliation's job, one layer up.
_Avoid_: unmanaged resource, out-of-band resource (see Discovered Resource), skew

**New-Child Drift**:
Drift where a managed parent resource (e.g. an `aws_security_group`) has gained an out-of-band child of one of the four FK-bearing child types (`aws_security_group_rule`, `aws_route`, `aws_route_table_association`, `aws_iam_role_policy_attachment`) — Sync ingests the child as an ordinary Discovered Resource exactly as it would any other unmanaged find (no separate write path), and a query-time Rule (`query.clj`'s `new-children-by-parent`) joins it back to its managed parent by typed value equality on the child's foreign-key attribute, generalized across every such relationship via a single data-driven `child-parent-joins` table (extending the `security-groups-with-port-22-open` join precedent). Surfaced under a `GET /drift` parent entry's `new_children` key.
_Avoid_: orphan child, unmanaged child

**Removed-Child Drift**:
Drift where a managed parent resource's previously Terraform-managed child of one of the four FK-bearing child types has disappeared out-of-band (deleted directly against the environment, not via `terraform destroy`). Detected via `:resource/sync-present?`, a plain boolean entity attribute Sync sets once per full pass (not per-resource) on every managed resource of a covered child type — `true` if that pass observed it, `false` if it didn't — deliberately excluded from `resource-pull-pattern`, so it is never read by `GET /state` reconstruction and never mutates live state (see ADR-0010). A query-time Rule (`query.clj`'s `removed-children-by-parent`) joins any child with `:resource/sync-present? false` back to its managed parent, the same `child-parent-joins` table New-Child Drift uses. Surfaced under a `GET /drift` parent entry's `removed_children` key. If the child later reappears in a Sync pass, the marker flips back to `true` (`resource-tx`'s reappearance fix) and it stops being flagged.
_Avoid_: deleted child, missing child, ghost resource
...
**Invocation**:
A Datomic entity recording one unattended `apply!`/`import!`/`destroy!` call (`infratomic.state-backend.terraform`, issue #33): which command ran, the resource address it targeted, and whether it succeeded — written unconditionally, on every call, regardless of whether the caller does anything with the returned result. Deliberately excludes captured stdout/stderr (Datomic dev-local's 4096-byte-per-string limit; the immediate caller still receives it in the returned `{:success ... :out ... :err ...}` map, just not durably). Distinct from a Violation (a Policy Check finding) and from Drift (a detected divergence) — an Invocation is an audit record of an execution attempt, not a finding.
_Avoid_: audit log entry, execution record, run record

**Lock** (resource address):
A Datomic entity (`infratomic.state-backend.terraform`, issue #33) serializing concurrent unattended `apply!`/`import!`/`destroy!` invocations against the same resource address — acquired via a CAS-guarded two-transaction sequence (never a single same-transaction upsert+CAS, see ADR-0011), held for the duration of one invocation, and released (the entity retracted) when it completes. A Lock older than a fixed TTL (10 minutes) is treated as abandoned (its holder presumed crashed) and becomes reacquirable without manual intervention. Scoped per resource address, not whole-state — two invocations targeting different addresses never contend.
_Avoid_: mutex, semaphore, latch

**Reconciliation**:
The State Backend operation (`infratomic.state-backend.reconcile`, issue #34) that runs automatically as the final step of every Sync pass (on-demand or scheduled) — evaluates every registered policy Rule directly against live state (not a speculative plan-derived db, unlike Policy Check), independent of Drift, and for every Resource found violating one, dispatches remediation by whether that specific resource is itself Terraform-managed: managed and drifted gets `terraform apply` (`terraform.clj`'s `apply!`, reused unmodified); not managed (a Discovered Resource, or the specific offending child of an otherwise-managed parent, e.g. a New-Child-Drift rule) gets synthesized import+destroy (Config Synthesis, below); managed and not drifted gets no remediation action, recorded only. A Rule that binds a parent resource but whose violation is attributable to a specific child (e.g. the port-22-open security group Rule) has that child resolved first, via a reconciliation-only companion query — the parent itself is never the remediation target for such a Rule. Every decision, remediated or not, is persisted as a `:reconciliation/*` entity (below). Closes the loop Drift/Discovered-Resource detection leaves open: nothing before this auto-acted on a detected policy bypass.
_Avoid_: auto-remediation, policy enforcement, drift correction

**`:reconciliation/*` entity**:
A Datomic entity (`db.clj`, issue #34) recording one Reconciliation decision: the violating Resource (`:reconciliation/resource`), the violated Rule's `:rule/id` (`:reconciliation/rule`), the action taken — `:reconciliation.action/apply`, `:reconciliation.action/import-destroy`, or `:reconciliation.action/none` (`:reconciliation/action`) — a timestamp (`:reconciliation/at`), and, when the action produced one, a reference to the Invocation entity (`:reconciliation/invocation`, absent for `:none`). Written unconditionally for every violating resource on every Reconciliation pass, including repeat `:none` records for a still-violating, still-non-drifted resource across many passes — no dedup, matching Invocation's own "written unconditionally, every call" precedent. Distinct from a Violation (a Policy Check finding, never persisted) and from an Invocation (an execution attempt, no notion of "why"): a `:reconciliation/*` entity is the one entity that is both a finding *and* a decision.
_Avoid_: audit record (see Invocation), violation record, remediation log

**Config Synthesis**:
The mechanism (`terraform.clj`'s `synthesize-import-and-destroy!`, issue #34, `terraform-config-synthesis` capability) that brings a previously-unmanaged resource (a Discovered Resource, or the specific offending child of an otherwise-managed parent) under Terraform's control just long enough to destroy it: a minimal `import { to = ... id = ... }` block is the only HCL this codebase ever authors directly — the resource's full configuration is generated by the provider itself via `terraform plan -generate-config-out`, never by this codebase mapping attributes to HCL. The import id is derived per resource type's id-space — a type's own stored AWS id directly (e.g. `aws_security_group`), or a synthesized Terraform composite id built from the resource's own stored attributes for a `db/id-space-mismatched-types` type (e.g. `aws_security_group_rule`, `aws_route`), never that type's raw stored `"id"` (a different id space for a Discovered instance). Runs entirely within a fresh scratch working directory, created per invocation and discarded unconditionally afterward regardless of success or failure — isolated from the shared working directory `apply!` reconciliation uses.
_Avoid_: import serialization, config generation, HCL synthesis
