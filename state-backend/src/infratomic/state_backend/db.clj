(ns infratomic.state-backend.db
  ...
  Backend. See docs/adr/0001-dual-storage-in-state-backend.md for the
  rationale behind storing both the raw state blob and per-resource
  entities."
...
(def schema
  [{:db/ident       :state-version/raw
    :db/valueType   :db.type/string
    :db/cardinality :db.cardinality/one
    :db/doc         "The exact raw Terraform state JSON as posted, verbatim."}
...
(defn latest-state-version-eid
  ...
  (let [max-tx (ffirst (d/q '[:find (max ?tx)
                               :where [?e :state-version/raw _ ?tx]]
                             db))]
    (when max-tx
      (ffirst (d/q '[:find ?e
                      :in $ ?tx
...
(defn all-state-version-eids
  ...
  still-live version."
  [db]
  (map first (d/q '[:find ?e :where [?e :state-version/raw]] db)))
